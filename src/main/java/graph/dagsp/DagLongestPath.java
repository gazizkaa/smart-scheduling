...
public class DagLongestPath {
    ...
    public static class Edge { public int to, weight; public Edge(int to, int weight) { this.to = to; this.weight = weight; } }
    ...
    public int[] longestPath(int source) {
        ...
        for (int i = 0; i < n; i++) if (!visited[i]) dfs(i, visited, topo);
        ...
        while (!topo.isEmpty()) {
            ...
            for (Edge e : adj.get(u)) {
                if (dist[u] != Integer.MIN_VALUE && dist[e.to] < dist[u] + e.weight) {
